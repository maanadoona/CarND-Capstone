...
class TLDetector(object):
    def __init__(self):
        ...
        self.sub_image = rospy.Subscriber('/image_color', Image, self.image_cb,
                                          queue_size=1)  # , buff_size=4*52428800)
        print("subscribed")
        ...
        while not rospy.is_shutdown():
            ...
            if not self.has_image:
                ...
                if self.current_pose and self.base_waypoints and len(self.lights) > 0 and len(self.stop_waypoints) > 0:
                    ...
                    if self.nlight_i >= 0 and (self.car_wp < self.stop_waypoints[self.nlight_i] + LOOK_BEHIND) and (
                            self.car_wp > self.stop_waypoints[self.nlight_i] - LOOK_AHEAD):
    ...
    def waypoints_cb(self, msg):
        ...
        for stop_pos in self.stop_positions:
            stop_pose_stamped = self.create_dummy_pose(stop_pos[0], stop_pos[1], stop_pos[2], stop_pos[3]) if len(
                stop_pos) == 4 \
    ...
    def image_cb(self, msg):
        ...
        if self.state != state:
            ...
            if state == TrafficLight.RED:
                print(("nlight_i:", self.nlight_i, "RED", self.state_count, "car_wp:", self.car_wp, "nlight_wp:",
                       self.nlight_wp))
            if state == TrafficLight.YELLOW:
                print(("nlight_i:", self.nlight_i, "YELLOW", self.state_count, "car_wp:", self.car_wp, "nlight_wp:",
                       self.nlight_wp))
    ...
    def get_closest_waypoint(self, pose):
        ...
        distances = np.array(
            [self.distance_sq(self_pos, way_pos.pose.pose.position) for way_pos in self.base_waypoints])
